...
import AheadMarker from "./AheadMarker";
...
const propTypes = {
   ...
   * A Boolean indicating whether we should cluster.
   * Default value is `false`
   *
   * @platform android
   */,
   ...
   */
    takeSnapshot(args) {
        ...
        if (config.result !== "file" && config.result !== "base64")
            throw new Error("Invalid result specified"); // Call native function
...
MapView.AheadMarker = AheadMarker;
